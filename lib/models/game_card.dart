...
/// Here we define what is a game. You will find the widgets use to display a game
/// and the function needed to get the informations
/// from the API.
...
class Game {
  ...
  displayGameInfo() {
    print(
        "The game is : $name of id $appid, cost is $price with rank : $rank , link is : $image, description is : $descriptionTxt\n");
  }
  ...
  setDetails(String url, String text, String gameName, String editor, gamePrice,
      String longText) {
    descriptionTxt = text;
    image = url;
    name = gameName;
  ...
  setRatings(List gameRatings) {
    for (var i in gameRatings) {
      ...
      i["review"] = i["review"].replaceAll("-", "");
      if (double.parse(i["weighted_vote_score"]) > 0.5 &&
          double.parse(i["weighted_vote_score"]) < 0.7) {
        i["weighted_vote_score"] = 3;
      } else if (double.parse(i["weighted_vote_score"]) > 0.7 &&
          double.parse(i["weighted_vote_score"]) < 0.8) {
...
class ItemWidget extends StatelessWidget {
                        ...
                        TextButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => InfoView(item: item)),
                            );
                          },
  ...
  /// the mainCard widget is slightly different from the previous one as this one is used to
  /// display the Most played game
            ...
            child: Column(
              ...
              children: [
                ...
                TextButton(
                  ...
                  child:
                      const Text('En savoir plus', textAlign: TextAlign.center),
                ),
              ],
            ),
          ),
